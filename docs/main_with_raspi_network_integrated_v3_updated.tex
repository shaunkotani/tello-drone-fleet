...
\section{クイックスタート：外部計測なしデモの準備手順}
\label{sec:quick_start_no_tracker}

本節では，外部計測なしデモを実行する直前の準備手順を短くまとめる．詳細なネットワーク設定，SSH/SCP，Tello Wi-Fi 接続確認，および各デモの判定方法は後続の節で説明する．ここでは，3 台の Raspberry Pi の有線 LAN 側 IP を \texttt{192.168.50.101}，\texttt{192.168.50.102}，\texttt{192.168.50.103}，全 Raspberry Pi のユーザー名を \texttt{inulab} とする．

\begin{enumerate}[label=\arabic*.]
  \item メイン PC の PowerShell から，各 Raspberry Pi へ SSH 接続する．gateway は各 Raspberry Pi 上で常駐させるため，Pi の台数分の SSH 画面を開いておく．
\begin{lstlisting}
ssh inulab@192.168.50.101
ssh inulab@192.168.50.102
ssh inulab@192.168.50.103
\end{lstlisting}
  各 SSH 画面で，Raspberry Pi 側の Python 仮想環境を有効化する．初回だけ \texttt{python3 -m venv ~/venv} を実行し，2 回目以降は \texttt{source ~/venv/bin/activate} だけでよい．
\begin{lstlisting}
...
\end{lstlisting}
  ...
  \item メイン PC 側の PowerShell で，\texttt{Tello-drone/python} に移動し，Python 仮想環境を有効化する．\texttt{Activate.ps1} が実行ポリシーで拒否される場合は，初回のみ \texttt{RemoteSigned} を \texttt{CurrentUser} スコープで許可する．
\begin{lstlisting}
...
python -m pip install --upgrade pip
\end{lstlisting}
  管理者権限や PC 全体の設定変更を避けたい場合は，その PowerShell セッションだけで次を実行してから仮想環境を有効化する．
\begin{lstlisting}
Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
.\venv\Scripts\Activate.ps1
\end{lstlisting}

  \item 3 台の Tello の電源を入れる．各 Raspberry Pi が対応する Tello Wi-Fi へ接続するまで少し待つ．各 SSH 画面で次を確認し，\texttt{TELLO-XXXXXX} と \texttt{192.168.10.1} への経路が見えることを確認する．
\begin{lstlisting}
iwgetid
...
\end{lstlisting}
  ...
  \item 各 Raspberry Pi の SSH 画面で gateway を起動する．Wi-Fi 接続が完了していない状態で起動すると \texttt{OSError: [Errno 101] Network is unreachable} になることがある．その場合は，\texttt{iwgetid} と \texttt{ip route get 192.168.10.1} を確認し，Tello Wi-Fi へ接続できてから同じコマンドを再実行する．
\begin{lstlisting}
source ~/venv/bin/activate
python3 tello_gateway.py --listen-host 0.0.0.0 --listen-port 10000 --auto-command
\end{lstlisting}
  gateway が起動している SSH 画面ではプロンプトが戻らない．その画面は閉じずに残し，デモスクリプトはメイン PC 側の別 PowerShell から実行する．

  \item メイン PC 側でデモスクリプトを実行する．まず非飛行の Demo 1 で通信を確認し，その後 Demo 2 の短時間 takeoff-hover-land へ進む．
\begin{lstlisting}
cd C:\Users\inulab\Work\Tello-drone\python
.\venv\Scripts\Activate.ps1
python .\no_tracker_demos\demo_01_comm_check.py --config ..\configs\no_tracker_demo_config.local.json --repeat 3
python .\no_tracker_demos\demo_02_takeoff_hover_land.py --config ..\configs\no_tracker_demo_config.local.json --duration 8 --rate-hz 10 --yes
\end{lstlisting}
\end{enumerate}
...
\begin{lstlisting}
scp raspberry_pi/tello_gateway.py inulab@192.168.50.101:~/tello_gateway.py
\end{lstlisting}

Pi が複数台ある場合は，各 Pi に同じ \texttt{tello\_gateway.py} をコピーする．Pi ごとの差分は，起動時の引数または接続先 Tello Wi-Fi の SSID で管理する．

\subsection{Raspberry Pi 側 Python 仮想環境の準備}

各 Raspberry Pi では，gateway 実行用の Python 仮想環境を用意してから \texttt{tello\_gateway.py} を起動する．仮想環境の作成は初回のみでよい．
...
以降，SSH 接続し直した場合は，gateway を起動する前に次を実行して仮想環境を有効化する．
...
PC の PowerShell から，有線 LAN 側 IP アドレスを指定して接続する．本実験で用いる Raspberry Pi のユーザー名は全台で \texttt{inulab} に統一しているため，SSH/SCP では \texttt{inulab@<Pi の有線 LAN 側 IP>} を指定する．
...
ここで，\texttt{192.168.50.101}，\texttt{192.168.50.102}，\texttt{192.168.50.103} はそれぞれ Pi-1，Pi-2，Pi-3 の有線 LAN 側 IP アドレスである．プロンプトが \texttt{inulab@inulab-rpi-01:~ \$} のように表示される場合，\texttt{@} の前の \texttt{inulab} がユーザー名，\texttt{@} の後の \texttt{inulab-rpi-01} がホスト名である．

ユーザー名を確認したい場合，Raspberry Pi にログイン済みの端末で次を実行する．

\begin{lstlisting}
whoami
\end{lstlisting}

本構成では，ここで \texttt{inulab} と表示されることを確認する．
...
scp .\tello_pi_gateway_patch\raspberry_pi\tello_gateway.py \
  inulab@192.168.50.101:~/tello_gateway.py
\end{lstlisting}
...
\texttt{scp} または \texttt{ssh} で \texttt{Permission denied} になる場合は，ユーザー名またはパスワードが違う可能性が高い．本実験では Raspberry Pi のユーザー名を \texttt{inulab} に統一しているため，次の形式で接続する．
...
プロンプトが次のように表示される場合，\texttt{inulab} がユーザー名，\texttt{inulab-rpi-01} がホスト名である．ホスト名をユーザー名として指定しないように注意する．

\begin{lstlisting}
inulab@inulab-rpi-01:~ $
\end{lstlisting}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item PC から \texttt{ssh inulab@192.168.50.101} でログインする．
...
\subsection{Python 仮想環境と PowerShell 実行ポリシー}

メイン PC 側では，既存プロジェクトの \texttt{python/} ディレクトリで仮想環境を作成し，有効化してからデモスクリプトを実行する．Windows PowerShell で \texttt{Activate.ps1} の実行が拒否される場合は，ユーザー単位で \texttt{RemoteSigned} を許可する．
...
すでに仮想環境を作成済みの場合，通常は次だけでよい．PowerShell のプロンプト先頭に \texttt{(venv)} が付けば有効化できている．
...
PC の実行ポリシーを恒久的に変更したくない場合は，現在の PowerShell セッションだけで一時的に許可してから有効化する．
...
\subsubsection{Step 0.5: メイン PC と Raspberry Pi の Python 仮想環境を有効化する}

メイン PC 側では，PowerShell で \texttt{Tello-drone/python} に移動し，仮想環境を有効化する．\texttt{Activate.ps1} が実行できない場合は，実行ポリシーを \texttt{CurrentUser} または \texttt{Process} スコープで許可してから再実行する．
...
各 Raspberry Pi 側では，メイン PC の PowerShell から SSH 接続し，SSH 先のシェルで仮想環境を有効化する．ユーザー名は全台で \texttt{inulab} である．
